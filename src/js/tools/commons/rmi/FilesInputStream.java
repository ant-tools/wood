...
/**
 * Files archive input stream. This class works together with {@link FilesOutputStream} to transfer files archives. A
 * files archive is a hierarchical files collections bundled with meta data. Archive is compressed using ZIP lossless
 * data compression and meta is carried by a standard Java JAR Manifest. This class gets a stream for files archive,
 * created by files output stream, and extract meta data and files hierarchy. It provides an iterator for files from
 * archive and an utility method to copy file to target system, see {@link #copy(File)}.
 * 
 * <p>
 * Common use case is with URL connection and Servlet HTTP request but is not limited to this scenario. Input stream for
 * this class could be a file archive stored on disk, but file content should be generated by files output stream. Both
 * output and input streams are processed on the fly.
 * 
 * <pre>
 ...
 * 
 * <p>
 * Manifest is mandatory even if no meta data is present and is always the first entry in archive. There is a single
 * predefined attribute, the implementation version but application level meta data is supported. Files output stream
 * can put arbitrary meta data using {@link FilesOutputStream#putMeta(String, Object)}. On receive side, this files
 * archive input stream can retrieve application meta data uing {@link #getMeta(String)} and related methods.
 * 
...
public class FilesInputStream extends InputStream implements Iterable<File>, Closeable
{
  ...
  /**
   * Archive manifest contains a single predefined attribute, namely implementation version. Application meta data can
   * be retrieved using {@link #getMeta(String)} and related methods.
   */
  ...
  public FilesInputStream(ZipInputStream filesArchive) throws IOException
  {
    ...
    if(entry == null) {
      throw new BugError("Missing manifest.");
    }
    ...
    try {
      ...
      if(!VERSION.equals(version)) {
        throw new BugError("Bad manifest version |%s|.", version);
      }
    }
    finally {
  ...
  public String getMeta(String key)
  {
    ...
    if(value == null) {
      throw new BugError("Missing |%s| attribute from manifest.", key);
    }
    return value;
  }
   ...
   * {@link FilesIterator} that takes care to update current ZIP entry. Usage pattern is in sample code; trying to use
   * it in different contexts has not predictable results.
   * 
   * <pre>
   * for(File file : files) {
   *   File targetFile = new File(targetDir, file.getPath());
   ...
   * related <code>targetFile</code> and enact files input stream to self copy its current entry to target file. Note
   * that this method ensure target file parent directories are created. Also this method works in overwrite mode, that
   * is, if target file already exist it will be overwritten and old content lost.
   * <p>
   * Warning: do not use {@link Files#copy(InputStream, OutputStream)}; it closes both input and output streams and
   * trying to get ZIP next entry after that will fail with <em>stream closed</em> exception.
   * 
  ...
  private class FilesIterator implements Iterator<File>
  {
    ...
    public boolean hasNext()
    {
      ...
      try {
        ...
        file = new File(path);
        if(file.isDirectory()) {
          throw new BugError("Target file |%s| is a directory.", file.getAbsolutePath());
        }
        return true;
      }
      catch(IOException e) {
        throw new BugError("Cannot read next entry: " + e.getMessage());
      }
    }
